...
import com.fgol.yeepee.BuildConfig
import com.fgol.yeepee.R
import com.fgol.yeepee.databinding.FragmentHomeBinding
import icu.andorid.yeepee.data.fetchLatestUpdate
...
import icu.andorid.yeepee.service.ServiceClient
import icu.andorid.yeepee.ui.activity.AboutActivity
import icu.andorid.yeepee.ui.util.ThemeUtils.getColor
import icu.andorid.yeepee.ui.util.ThemeUtils.themeColor
import icu.andorid.yeepee.ui.util.makeToast
import icu.andorid.yeepee.ui.util.navController
import icu.andorid.yeepee.ui.util.setupToolbar
...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private val backupSAFLauncher =
        registerForActivityResult(CreateDocument("application/json")) backup@{ uri ->
            ...
            ConfigManager.configFile.inputStream().use { input ->
                yepApp.contentResolver.openOutputStream(uri).use { output ->
    ...
    private val restoreSAFLauncher =
        registerForActivityResult(ActivityResultContracts.GetContent()) restore@{ uri ->
            ...
            runCatching {
                val backup = yepApp.contentResolver
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.detectionTest.setOnClickListener {
            val intent = yepApp.packageManager.getLaunchIntentForPackage("icu.andorid.applistdetector")
            if (intent == null) {
                MaterialAlertDialogBuilder(requireContext())
        ...
        binding.backupConfig.setOnClickListener {
            backupSAFLauncher.launch("YEP_Config.json")
        }
    ...
    override fun onStart() {
        ...
        val color = when {
            !yepApp.isHooked -> getColor(R.color.gray)
        ...
        if (serviceVersion != 0) {
            if (serviceVersion < icu.andorid.yeepee.common.BuildConfig.SERVICE_VERSION) {
