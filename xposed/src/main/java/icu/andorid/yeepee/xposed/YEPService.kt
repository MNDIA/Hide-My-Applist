package icu.andorid.yeepee.xposed

import android.content.pm.ApplicationInfo
import android.content.pm.IPackageManager
import android.os.Build
import icu.andorid.yeepee.common.*
import icu.andorid.yeepee.xposed.hook.*
import java.io.File

class YEPService(val pms: IPackageManager) : IYEPService.Stub() {

    companion object {
        private const val TAG = "YEP-Service"
        var instance: YEPService? = null
    }
    ...
    init {
        ...
        loadConfig()
        installHooks()
        logI(TAG, "YEP service initialized")
    }

    private fun searchDataDir() {
        File("/data/misc/yhide_my_applist").deleteRecursively()
        File("/data/system").list()?.forEach {
            if (it.startsWith("yhide_my_applist")) {
        ...
        if (!this::dataDir.isInitialized) {
            dataDir = "/data/system/yhide_my_applist_" + Utils.generateRandomString(16)
        }
    ...
    fun isVoldEnabled(packageName: String) {
        ...
        var vold = appConfig.vold ?: return false
        return vold
    }
    ...
    fun shouldHide(caller: String?, query: String?): Boolean {
        ...
        if ((caller == Constants.GMS_PACKAGE_NAME || caller == Constants.GSF_PACKAGE_NAME) && query == Constants.APP_PACKAGE_NAME) return false // If apply hide on gms, yep app will crash 😓
